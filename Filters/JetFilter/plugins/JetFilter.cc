...
class JetFilter : public edm::EDFilter {
   ...
   private:
	...
	// ?
	EDGetTokenT<std::vector<pat::Jet>> jetCollection_;
...
JetFilter::JetFilter(const edm::ParameterSet& iConfig):
	...
	//?
    //?????????????????
	//jetCollection_(consumes<std::vector<pat::Jet>>(iConfig.getParameter<InputTag>("jetCollection"))),
    jetCollection_(consumes<std::vector<pat::Jet>>(iConfig.getParameter<InputTag>("jetCollection"))),
...
JetFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	cout << iEvent.getByToken(triggerResults_, results) << endl;
	Handle<pat::PackedTriggerPrescales> prescales;
	cout << iEvent.getByToken(triggerPrescales_, prescales) << endl;
	...
	// Get CA12 jets:
	cout << "jets1" << endl;
	Handle<std::vector<pat::Jet>> jets_edm;
	cout << "jets2" << endl;
    cout << iEvent.getByToken(jetCollection_, jets_edm) << endl;
	cout << "jets3" << endl;

	// Ensure the top two jets have pT greater than cut_pt_:
	// for (std::vector<std::vector<pat::Jet>>::const_iterator jet = jets_edm->begin(); jet != jets_edm->end(); ++ jet) {
	// 	cout << jet->pt() << endl;
	// }
