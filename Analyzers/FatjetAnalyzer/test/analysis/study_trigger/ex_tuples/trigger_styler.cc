...
//	TH2* pass = (TH2*) tf->Get("BoostedTriggerEfficiencySeveralTriggers/jet1PrunedMassHTPassing_cutDijet");
	...
	TH2* total_ht_m = (TH2*) tf->Get("ht_mp0_total");
	TH2* total_ht_mak8 = (TH2*) tf->Get("ht_mp0ak8_total");
	TH2* pass_ht_m = (TH2*) tf->Get("ht_mp0_pass");
	TH2* pass_ht_mak8 = (TH2*) tf->Get("ht_mp0ak8_pass");
	TH1* total_ht = total_ht_m->ProjectionX("total_ht");
...
//	TH1* total_mak8 = total_ht_mak8->ProjectionY("total_mak8", total_ht_mak8->GetXaxis()->FindBin(900), total_ht_mak8->GetNbinsX());
	TH1* pass_ht = pass_ht_m->ProjectionX("pass_ht");
...
//	
	...
	for (unsigned i = 0; i < algos.size(); ++i) {
		...
		TString name = "eff_m" + algo;
		TCanvas* tc_m = new TCanvas(name, name);
		
		TString algo_temp = "";
		if (i != 0) algo_temp = algo;
		TH2* total = (TH2*) tf->Get("ht_mp0" + algo_temp + "_total");
		TH2* pass = (TH2*) tf->Get("ht_mp0" + algo_temp + "_pass");
		
		TH1* total_m = total->ProjectionY("total_m", total->GetXaxis()->FindBin(cut_ht), total->GetNbinsX());
		TH1* pass_m = pass->ProjectionY("pass_m", pass->GetXaxis()->FindBin(cut_ht), pass->GetNbinsX());
