...
def make_pfjet_collection_pruned(process, sequence, pfjet_tag, pfcon_tag, algo, pum, groom):
	# Make jets:
	tag_jets = pfjet_tag + groom.title
	jet_producer = getattr(
		PFJets,
#		next(f for f in dir(PFJets) if f == "ak4PFJets{}{}".format(pum.title, groom.title))		# This should raise an exception if nothing matches.
		next(f for f in dir(PFJets) if f == "ak4PFJets{}".format(pum.title))		# This should raise an exception if nothing matches.
	).clone(		# ak8 or ak4 doesn't matter when "Pruning" is left off.
#		src=cms.InputTag(pfcon_tag + ":constituents"),
		src=cms.InputTag(pfcon_tag),
		rParam=algo.r,
		jetAlgorithm=algo.title,
		doAreaFastjet=cms.bool(True),
#		useExplicitGhosts = cms.bool(True),
#		zCut=cms.double(groom.z),
#		rCutFactor=cms.double(groom.r),
#		writeCompound = cms.bool(True),		# Breaks things ...
	)
	...
	# Recover constituents:
	tag_cons = pfcon_tag + groom.title
	con_producer = cms.EDFilter("MiniAODJetConstituentSelector",
		src=cms.InputTag(tag_jets),
		cut=cms.string("")
	)
	...
	return (tag_jets, tag_cons)




def make_pfjet_collection_filtered(process, sequence, pfjet_tag, pfcon_tag, algo, pum, groom):
	# Make jets:
	tag_jets = pfjet_tag + groom.title
	jet_producer = getattr(
		PFJets,
		next(f for f in dir(PFJets) if f == "ak8PFJets{}{}".format(pum.title, groom.title))		# This should raise an exception if nothing matches.
	).clone(
		src=cms.InputTag(pfcon_tag + ":constituents"),
		rParam=algo.r,
		jetAlgorithm=algo.title,
